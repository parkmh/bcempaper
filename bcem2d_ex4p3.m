...
% Block-regular Grids" by M. Park and M.V. Tretyakov. See Example 4.2.
...
L  = [real(reshape(fc(:,:,1),mbar,1)) reshape(fc(:,:,2),mbar,1) ...
    conj(reshape(fc(:,:,2),mbar,1)) real(reshape(fc(:,:,1),mbar,1))]; % Note that the diagonal entries are real
